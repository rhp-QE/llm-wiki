...
1. Intake: identify inputs, preserve source evidence, check compatible existing source groups, and stop if required source preservation cannot be satisfied.
...
3. Route: choose source type, target domains, templates, existing source file to append or new source destination, and existing pages to update.
...
8. Maintenance: check links, frontmatter, aliases, duplicates, source grouping, time-sensitive claim dates, status, indexes, and processed inbox cleanup.
...
- Incremental updates must check existing sources before creating a new source file. If the same date, primary subject, semantic topic, source type/content form, and origin are compatible, append the update as a new fragment/section in the existing source file instead of creating a duplicate source.
- Same-source appended fragments must keep their own fragment ID, captured date, effective/valid-as-of date when relevant, origin/context, and verbatim raw payload.
...
- Every non-trivial compiled claim needs a clickable source Markdown link, explicit user statement, `Inference`, or `Needs evidence`.
- Time-varying compiled claims need an "as of" date, timeline entry, or current-state timestamp.
- Every mutating ingest must verify that newly introduced or edited local Markdown links resolve to the intended files.
